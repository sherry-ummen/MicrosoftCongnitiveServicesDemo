...
namespace ComputerVisionAzure {
    /// <summary>
    /// Taken from this article https://msdn.microsoft.com/en-us/magazine/dn630647.aspx
    /// Required for async command operations and also helpful in testing those
    /// </summary>
    ...
    public static class AsyncCommand {
        public static AsyncCommand<object> Create(Func<Task> command) {
            return new AsyncCommand<object>(async (token, input) => { await command(); return null; });
        }

        public static AsyncCommand<object> Create(Func<object, Task> command) {
            return new AsyncCommand<object>(async (token, input) => { await command(input); return null; });
        }
    }
}
